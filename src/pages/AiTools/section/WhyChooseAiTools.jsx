    ...
    <section className="w-full py-6 lg:py-16 px-6 md:px-12 bg-primary rounded-3xl shadow-sm relative overflow-hidden">
      ...
      {data?.section_title && (
        <div className="text-center mb-8 md:mb-14 relative z-10">
          ...
          <div className="w-26 h-1 bg-secondary mx-auto mt-2 md:mt-4 rounded-full" />
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 xl:gap-8 relative z-10">
        {data?.items?.map((feature) => (
          ...
          >
            <div
              className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-5 transition-all duration-300 bg-slate-300 group-hover:bg-secondary/50`}
            >
              {feature.icon && (
                <img
                  src={feature.icon}
